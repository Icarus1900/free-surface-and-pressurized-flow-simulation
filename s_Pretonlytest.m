function [Q1,Z1] = s_Pretonlytest ( na , lt , wch , hch , nt , n2 , P1 , Pn , DT , Q , Z , Bsl )  %%Qm/Am更换为Vm
G = 9.8;
Emax = 0.1;
R = 0.85;
N = na;
...
while norm ( ZQ1 - ZQ2 ) > Emax||temp <= 1                              %当ZQ1与ZQ2足够相似时停止计算
    ...
    for I = 1 : N - 1                             %计算方格编号
        ...
        Qt ( 2 , 1 ) = Q ( I );
        Qt ( 2 , 2 ) = Q ( I + 1 );
        [Am,Bm,Qm,Pm,Am1,Am0,Vm]=feval ( @s_prettest , R , wch , hch , Qt , Zt , Bsl );
        c1=2*R*(DT/DS)/Bm;                                         %R：系数θ
        e1=Z(I)+Z(I+1)+((1-R)/R)*c1*(Q(I)-Q(I+1));
        a2=2*R*(DT/DS)*((Vm)^2*Bm-G*Am);
        c2=1-4*R*(DT/DS)*(Vm);
        d2=1+4*R*(DT/DS)*(Vm);
        e2=((1-R)/R)*a2*(Z(I+1)-Z(I))+(1-4*(1-R)*(DT/DS)*(Vm))*Q(I+1)+(1+4*(1-R)*(DT/DS)*(Vm))*Q(I);
        e2=e2+2*DT*(Vm)^2*(Am1-Am0)/DS-2*DT*(G*n2^2*Qm^2*Pm^(4/3))/(Am^(7/3));
